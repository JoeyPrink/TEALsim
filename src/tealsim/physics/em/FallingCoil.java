/* $Id: FallingCoil.java,v 1.21 2010/09/22 15:48:11 pbailey Exp $ */
/**
 * A 
 * 
...
public class FallingCoil extends SimEM {
	...
	public FallingCoil() {
		...
		setDeltaTime(0.01); //sets the time step of the simulation.
		setDamping(0.); //sets the generalized, velocity-based damping on the system.
...
/*
			...
			cylN.setGeometry(teal.render.j3d.geometry.Cylinder.makeGeometry(16, 0.3, 2.0));
			...
			*/
			...
			// This will be the wooden base holding the magnet.
			modelBase = new Rendered();
			Model base = new Model("models/Main_Base_at_Zero.3DS");
			base.setScale(metricScale);
			modelBase.setModel(base);
			modelBase.setPosition(new Vector3d(0., -2.45, 0.));
			addElement(modelBase);

			// Here we load an external model and set it to be the model used by the RingOfCurrent.
			// This is a model of a coil of wire that replaces the generic "torus" that is used as the model by default.
			Model coil = new Model("models/Coil_at_Zero.3DS");
			coil.setScale(metricScale);
			roc.setModel(coil);

			// Here we create a Rendered object to hold a cylinder that we generate internally.
			// This will be the wooden cylinder that the magnet rests on.
			modelMagBase = new teal.render.primitives.Cylinder(0.3, 2.0);
			modelMagBase.setColor(new Color(160, 140, 110));
			//TealMaterial mat = new TealMaterial(new Color(160, 140, 110));
			//mat.setTransparancy(0);
			//modelMagBase.setMaterial(mat);
			modelMagBase.setPosition(new Vector3d(0, -1.30, 0));
			addElement(modelMagBase);

			// Here we load an external model and set it to be the model used by the MagneticDipole.
			// This is a model of a magnet (a silver chamfered cylinder) that replaces the generic cylinder used by default.
			Model magModel = new Model("models/Magnet_At_Zero.3DS");
			magModel.setScale(metricScale);
			magModel.setOffset(new Vector3d(0.,-.5,0.));
			mag.setModel(magModel);
...
//		RectangularPlane recTst = new RectangularPlane(new Vector3d(-1., -1., -1.),
//				new Vector3d(-1., 1., 1.), new Vector3d(1., 1., 1.));
//		fv.setGridIterator(recTst);
