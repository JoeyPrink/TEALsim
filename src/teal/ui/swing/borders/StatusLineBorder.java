 ...
 * @author  Dafe Simonek
 */
public class StatusLineBorder extends AbstractBorder {

    private static final long serialVersionUID = 3617006451453801781L;
    ...
    public void paintBorder(Component c, Graphics g, int x, int y, int w, int h) {
        g.translate(x, y);
        Color borderC = UIManager.getColor(Color.gray);
        g.setColor(borderC);
        // top
        if ((type & TOP) != 0) {
    ...
    public Insets getBorderInsets(Component c) {
        if (insets == null) {
            insets = new Insets((type & TOP) != 0 ? 1 : 0, (type & LEFT) != 0 ? 1 : 0, 0, (type & RIGHT) != 0 ? 1 : 0);
        }
        return insets;
    }
