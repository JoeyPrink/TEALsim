 ...
 * 
 * The order of evaluation of the states is: 1 Test if button is selected 1.1
 * Test if button is disabled 1.2. Test if button is pressed 1.3. Test if button
 * is in rollover state 1.4. Button must be in normal state then. 2. If not
 * selected 2.1 Test if button is disabled 2.2. Test if button is pressed 2.3.
 * Test if button is in rollover state 2.4. If checkForDefaultButton is true
 * there check whether it's the default button. If it's not or
 * checkForDefaultButton is false return the normal state.
 * 
 * The first test that evaluates to true determines the index of the skin to be
 * used.
 * 
 * If the component is not a subclass of AbstractButton one can explicitly pass
 * the relevant states to getIndexForState but use the same logic.
 */
public class SkinToggleButtonIndexModel {
	 ...
	 * rollover=1, pushed=2 and disabled=3, selected=4, selectedRollover=5,
	 * selectedPressed=6, selectedDisabled=7; There's no check concering the
	 * default button
	 */
	 ...
	 * Creates a SkinIndexModel for the button with the indices normal=0,
	 * rollover=1, pushed=2 and disabled=3, selected=4, selectedRollover=5,
	 * selectedPressed=6, selectedDisabled=7;
	 * 
	 * @param checkForDefaultButton
	 ...
	 * 
	 */
	public SkinToggleButtonIndexModel(int normal, int rollover, int pressed, int disabled, int selected, int selectedRollover, int selectedPressed,
	...
	/**
	 * @see teal.ui.swing.skin.SkinOffsetModel#getIndexForState()
	 */
