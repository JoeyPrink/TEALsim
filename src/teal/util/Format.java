...
public class Format {
     ...
     * s is the format string following printf conventions
     ...
     * </dl>
     * </ul>
     * IllegalArgumentException  exception if bad format
     */
    ...
    private String sign(int s, String r) {
        ...
        if (leading_zeroes)
            w = width;
        else if ((fmt == 'd' || fmt == 'i' || fmt == 'x' || fmt == 'X' || fmt == 'o') && precision > 0) w = precision;
    ...
    private String fixed_format(double d) {
        ...
        if (precision == 0) return (long) (d + 0.5) + (removeTrailing ? "" : ".");
    ...
    private String exp_format(double d) {
        ...
        if ((fmt == 'g' || fmt == 'G') && e >= -4 && e < precision) return fixed_format(d);
